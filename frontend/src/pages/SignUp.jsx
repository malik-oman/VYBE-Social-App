  ...
  return (
    <div className="w-full min-h-screen bg-gradient-to-br from-slate-950 via-gray-900 to-slate-800 flex flex-col justify-center items-center p-4">
      <div className="w-full max-w-[1100px] min-h-[620px] bg-white/95 backdrop-blur-sm rounded-3xl flex justify-center items-center overflow-hidden shadow-2xl shadow-black/40 border border-white/10">
        ...
        <div className="w-full lg:w-[50%] min-h-[620px] bg-white flex flex-col items-center p-6 gap-5">
          <div className="flex gap-2 items-center text-[22px] font-bold mt-8 tracking-tight">
            <span className="text-gray-800">Sign Up to</span>
            <img src={logo} alt="VYBE" className="w-[75px] object-contain" />
          </div>

          <p className="text-gray-400 text-sm font-medium -mt-2">Create your account to get started</p>
          ...
          >
            <label
              htmlFor="name"
              className={`text-gray-500 absolute left-4 px-1 bg-transparent text-[13px] font-medium transition-all duration-300 pointer-events-none ${inpuClicked.name || name ? "-top-2.5 text-xs text-black bg-white" : "top-1/2 -translate-y-1/2"} `}
            >
              Enter Your Name
            </label>
            <input
              ...
              onFocus={() => setInputClicked({ ...inpuClicked, name: true })}
              onBlur={() => !name && setInputClicked({ ...inpuClicked, name: false })}
            />
          </div>
